...
async function onClickTracking(trackingInfo) {
    ...
    paymentUpdate.type = "USING"
  	...
  	const payment = await getAssetRegistry('org.vela.gas.Gas');
  	await payment.update(trackingInfo.paymentWallet);
    ...
    rewardUpdate.type = "REWARD"
  	...
  	const reward = await getAssetRegistry('org.vela.gas.Gas');
  	await reward.update(trackingInfo.rewardWallet);
}
